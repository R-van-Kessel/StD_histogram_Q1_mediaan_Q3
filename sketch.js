...
function generateQuestions() {
    ...
    // Genereer meer histogrammen dan nodig (20 stuks)
    let allHistograms = [];
    for (let i = 0; i < 20; i++) {
        let frequencies = [];
        for (let j = 0; j < 6; j++) {
        ...
        let stats = berekenStatistieken(frequencies);
    ...
    // Groepeer histogrammen op gemiddelde (afgerond)
    let groepenPerGemiddelde = {};
    for (let hist of allHistograms) {
        let gem = hist.stats.gemiddelde;
        if (!groepenPerGemiddelde[gem]) {
            groepenPerGemiddelde[gem] = [];
        }
        groepenPerGemiddelde[gem].push(hist);
    }
    
    // Selecteer 10 histogrammen, waarbij we proberen paren te maken met hetzelfde gemiddelde
    let geselecteerdeHistogrammen = [];
    let gebruikteGemiddeldes = [];
    
    // Eerst: zoek gemiddeldes met minimaal 2 histogrammen
    for (let gem in groepenPerGemiddelde) {
        if (groepenPerGemiddelde[gem].length >= 2 && geselecteerdeHistogrammen.length < 10) {
            // Kies 2 histogrammen met dit gemiddelde
            let paar = groepenPerGemiddelde[gem].slice(0, 2);
            
            // Check of ze verschillende kwartielen hebben
            if (paar[0].stats.q1 !== paar[1].stats.q1 || 
                paar[0].stats.mediaan !== paar[1].stats.mediaan || 
                paar[0].stats.q3 !== paar[1].stats.q3) {
                geselecteerdeHistogrammen.push(paar[0]);
                geselecteerdeHistogrammen.push(paar[1]);
                gebruikteGemiddeldes.push(gem);
            }
        }
    }
    
    // Vul aan met willekeurige histogrammen tot we 10 hebben
    for (let hist of allHistograms) {
        if (geselecteerdeHistogrammen.length >= 10) break;
        
        // Voeg alleen toe als deze nog niet geselecteerd is
        if (!geselecteerdeHistogrammen.includes(hist)) {
            geselecteerdeHistogrammen.push(hist);
        }
    }
    
    // Als we nog steeds niet genoeg hebben, genereer extra
    while (geselecteerdeHistogrammen.length < 10) {
        let frequencies = [];
        for (let j = 0; j < 6; j++) {
            frequencies.push(floor(random(0, 7)));
        }
        let stats = berekenStatistieken(frequencies);
        geselecteerdeHistogrammen.push({
            frequencies: frequencies,
            stats: stats
        });
    }
    
    // Bouw vragen en antwoorden
    for (let i = 0; i < 10; i++) {
        let hist = geselecteerdeHistogrammen[i];
        
        questions.push({
            text: "HISTOGRAM",
            histogram: hist.frequencies,
            answer: "gem:" + hist.stats.gemiddelde + "|q1:" + hist.stats.q1 + "|med:" + hist.stats.mediaan + "|q3:" + hist.stats.q3,
            answerId: i
        });
        
        answers.push({
            text: "\n Gemiddelde: " + hist.stats.gemiddelde + "\n" +
                  " Q1: " + hist.stats.q1 + "\n" +
                  " Mediaan: " + hist.stats.mediaan + "\n" +
                  " Q3: " + hist.stats.q3,
            answer: "gem:" + hist.stats.gemiddelde + "|q1:" + hist.stats.q1 + "|med:" + hist.stats.mediaan + "|q3:" + hist.stats.q3,
            answerId: i
        });
...
// Hulpfunctie om statistieken te berekenen
function berekenStatistieken(frequencies) {
    let totaal = 0;
    let som = 0;
    let alleWaardes = [];
    ...
    let gemiddeldeStr = gemiddelde.toString().replace('.', ',');
    ...
    let mediaanStr = mediaan.toString().replace('.', ',');
    ...
    if (alleWaardes.length === 0) {
        ...
        let lowerHalf = alleWaardes.slice(0, floor(alleWaardes.length / 2));
        if (lowerHalf.length === 0) {
            q1 = alleWaardes[0];
        } else if (lowerHalf.length % 2 === 0) {
            q1 = (lowerHalf[lowerHalf.length/2 - 1] + lowerHalf[lowerHalf.length/2]) / 2;
        } else {
            q1 = lowerHalf[floor(lowerHalf.length/2)];
        }
    }
    q1 = round(q1 * 10) / 10;
    let q1Str = q1.toString().replace('.', ',');
    ...
    if (alleWaardes.length === 0) {
        ...
        let upperHalf = alleWaardes.slice(ceil(alleWaardes.length / 2));
        if (upperHalf.length === 0) {
            q3 = alleWaardes[alleWaardes.length - 1];
        } else if (upperHalf.length % 2 === 0) {
            q3 = (upperHalf[upperHalf.length/2 - 1] + upperHalf[upperHalf.length/2]) / 2;
        } else {
            q3 = upperHalf[floor(upperHalf.length/2)];
        }
    }
    q3 = round(q3 * 10) / 10;
    let q3Str = q3.toString().replace('.', ',');
    
    return {
        gemiddelde: gemiddeldeStr,
        mediaan: mediaanStr,
        q1: q1Str,
        q3: q3Str,
        gemiddeldeNumeriek: gemiddelde,
        totaal: totaal,
        som: som
    };
}
